...
module Outcome = struct
  (** The return value of {!all_settled} representing the fulfillment status of
      a Promise. *)
  ...
  (** The resolved value of the Promise. Only present if the Promise is
      fulfilled. *)

  external reason : _ t -> 'err option = "reason"
  [@@mel.get] [@@mel.return undefined_to_opt]
  (** The rejected value of the Promise. Only present if the Promise is
      rejected. *)
end

external all_settled : 'a t array -> 'a Outcome.t array t = "allSettled"
[@@mel.scope "Promise"]
(** Takes an array of Promises as input. Returns a single Promise that resolves
    ...
    an array of {!Outcome.t} containing the status of each Promise in the input.
*)
