...
let parse_query command params =
  ...
  | "CLIENTINFO", Some commands ->
      Client_info_reply
        (String.split_re ~pattern:[%mel.re {re|/\s+/|re}] commands
        |> Utils.keep_some)
...
let string_of_query = function
  ...
  | Client_info_reply commands -> "CLIENTINFO " ^ Array.join ~sep:" " commands
  | Finger_query -> "FINGER"
...
let delim = "\001"
let is_ctcp msg = String.starts_with ~prefix:delim msg

let parse message =
  if not (is_ctcp message) then None
  else
    let message =
      if String.ends_with ~suffix:delim message then
        String.slice ~start:1 ~end_:(String.length message - 1) message
      else String.slice ~start:1 message
    in
