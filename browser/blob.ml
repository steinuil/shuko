...
external init :
  ?type_:string -> ?endings:[ `transparent | `native ] -> unit -> init = ""
[@@mel.obj]

external create : _ -> init -> t = "Blob" [@@mel.new]

let of_blob ?type_ ?endings (blob : t) = create blob (init ?type_ ?endings ())

let of_buffer_source ?type_ ?endings (buffer_source : Buffer_source.t) =
  create buffer_source (init ?type_ ?endings ())

let of_string ?type_ ?endings (string : string) =
  create string (init ?type_ ?endings ())

let create ?type_ ?endings () = create Js.undefined (init ?type_ ?endings ())
